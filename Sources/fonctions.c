...
{
    int **add = NULL;
    add = contour_prewitt_sobel(image,dim[0],dim[1],6);
    int indLig,X;
    ...
    int **add = NULL;
    int indLig,X;
    imageC = contour_prewitt_sobel(image,dim[0],dim[1],3);
    imageC2 = contour_prewitt_sobel(image,dim[0],dim[1],4);
    add = addition(image,image,dim,dim);*/

    //Creation d'une matrice de vote
    int **imageVote = NULL;
    int lig = sqrt( (dim[0] * dim[0]) + (dim[1] * dim[1]) );
    imageVote = malloc(lig *sizeof(int*));
    for (int i = 0; i < lig; i++)
    {
       imageVote[i] = malloc(181 * sizeof(int));
       for(int j=0;j<181;j++)
       {
           imageVote[i][j] = 0;
       }
    ...
    for(int i=0;i<dim[0];i++)
    {
        for(int j=0;j<dim[1];j++)
        {
            if(add[i][j]>=240)
            {
                for(int k=0;k<181;k++)
                {
                    indLig = i*cos((k*3.14)/180) + j*sin((k*3.14)/180);
                    if(indLig<lig && indLig>=0)
                    {
                        imageVote[indLig][k]++;
                    }
                }
    ...
    //Initialisation et Construction de l'image contenant la droite contour
    //**Initialisation
    int **imageDroite = NULL;
    imageDroite = malloc(dim[0] *sizeof(int*));
    for (int i = 0; i < dim[0]; i++)
    {
       imageDroite[i] = malloc(dim[1] * sizeof(int));
       for(int j=0;j<dim[1];j++)
       {
           imageDroite[i][j] = 0;
       }
    ...
    for(int i=0;i<lig;i++)
    {
        for(int j=0;j<181;j++)
        {
            if(imageVote[i][j]>=108)
            {
                for(int k=0;k<dim[1];k++)
                {
                    X = ( i - (k*sin((j*3.14)/180)) ) / cos((j*3.14)/180);
                    if(X<dim[0] && X>=0)
                    {
                        imageDroite[X][k] = 255;
                    }
    ...
    int **imageAdd = NULL;
    imageAdd = addition(image, imageDroite, dim, dim);
    Tuple3 t = {imageAdd,dim[0], dim[1]};

    //Tuple3 t = {imageDroite,dim[1], dim[0]};
    //Tuple3 t = {imageVote, lig,181};
    return t;
}
...
{
    int **imageT = NULL;
    imageT = malloc(200 * sizeof(int *));
    for (int i = 0; i < 200; i++)
    {
        imageT[i] = malloc(300 * sizeof(int));
        for(int j=0;j<300;j++)
        {
            imageT[i][j] = 120;
        }
